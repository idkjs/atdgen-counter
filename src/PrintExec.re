...
let to_string = x => {
  let rec conv =
    ...
    | [] =>
      ...
      | [@implicit_arity] Undefined_recursive_module(file, line, char) =>
        sprintf(
          locfmt,
          file,
          line,
...
type backtrace_slot =
  ...
  | Unknown_location({is_raise: bool});
...
let convert_raw_backtrace_slot: raw_backtrace_slot => backtrace_slot = (
  _ => failwith("convert_raw_backtrace_slot not implemented"):
...
/* #else
   ...
   #end */
external convert_raw_backtrace: raw_backtrace => array(backtrace_slot) =
...
let print_exception_backtrace = (outchan, backtrace) =>
  ...
  | Some(a) =>
    for (i in 0 to Array.length(a) - 1) {
      switch (format_backtrace_slot(i, a[i])) {
      | None => ()
...
let backtrace_slots = raw_backtrace =>
  /* The documentation of this function guarantees that Some is
     ...
     themselves. */
  switch (convert_raw_backtrace(raw_backtrace)) {
  | None => None
  | Some(backtrace) =>
    let usable_slot = (
      fun
